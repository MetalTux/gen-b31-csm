...
export default function Sidebar({ userRole }: { userRole: string }) {
  ...
  useEffect(() => {
    ...
    // Usamos setTimeout para evitar el renderizado en cascada síncrono (soluciona el warning)
    const timeoutId = setTimeout(() => {
      handleResize();
    }, 0);

    // Escuchamos si el usuario redimensiona la ventana o gira su tablet
  ...
  useEffect(() => {
    // Nuevamente usamos setTimeout para evitar el error de React
    const timeoutId = setTimeout(() => {
      if (window.innerWidth < 768) {
        setIsOpen(false);
      }
