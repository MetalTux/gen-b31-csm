...
export default function Sidebar({ userRole }: { userRole: string }) {
  ...
  // Función auxiliar para saber si un link está activo y pintarlo diferente
    ...
    <>
      ...
      >
        ...
        <div className="p-6 text-center border-b border-white/10 bg-black/20 relative shrink-0">
          ...
          {/* --- AQUÍ REEMPLAZAMOS CSM POR TU NUEVO LOGO --- */}
          <div className="relative w-28 h-32 mx-auto mb-2 drop-shadow-xl hover:scale-105 transition-transform duration-300">
            <Image 
              src="/logocolegio.png" // Apunta directo al archivo que guardaste en la carpeta public/
              alt="Logo Portal Apoderados Colegio San Marcos"
              fill
              className="object-contain"
              sizes="(max-width: 768px) 100vw, 150px"
              priority // Prioridad alta para que cargue rápido al abrir la página
            />
          </div>
          {/* --- FIN DEL LOGO --- */}
        ...
        <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto custom-scrollbar">
          ...
          <Link 
            ...
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${
              isActive("/") ? "bg-brand-accent text-brand-navy shadow-md" : "text-gray-300 hover:bg-white/10 hover:text-white"
            }`}
          >
          ...
          <Link 
            ...
            href="/mis-pagos" 
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${
              isActive("/mis-pagos") ? "bg-brand-accent text-brand-navy shadow-md" : "text-gray-300 hover:bg-white/10 hover:text-white"
            }`}
          >
          ...
          {/* Menú Exclusivo de Administración */}
          {userRole === "ADMIN" && (
            <>
              ...
              <Link 
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all mt-4 border border-white/10 ${
                  isActive("/admin/configuracion") ? "bg-white text-brand-navy shadow-md" : "text-gray-300 hover:bg-white/10 hover:text-white"
                }`}
              >
              ...
              <Link 
                ...
                href="/admin/usuarios" 
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${
                  isActive("/admin/usuarios") ? "bg-white text-brand-navy shadow-md" : "text-gray-300 hover:bg-white/10 hover:text-white"
                }`}
              >
              ...
              <Link 
                ...
                href="/admin/alumnos" 
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${
                  isActive("/admin/alumnos") ? "bg-white text-brand-navy shadow-md" : "text-gray-300 hover:bg-white/10 hover:text-white"
                }`}
              >
              ...
              <Link 
                ...
                href="/admin/ingresos" 
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${
                  isActive("/admin/ingresos") ? "bg-white text-brand-navy shadow-md" : "text-gray-300 hover:bg-white/10 hover:text-white"
                }`}
              >
              ...
              <Link 
                ...
                href="/admin/egresos" 
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${
                  isActive("/admin/egresos") ? "bg-white text-brand-navy shadow-md" : "text-gray-300 hover:bg-white/10 hover:text-white"
                }`}
              >
