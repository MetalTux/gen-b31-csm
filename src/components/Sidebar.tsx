...
export default function Sidebar({ userRole }: { userRole: string }) {
  // 1. Iniciamos en false para evitar que cubra la pantalla en móviles al cargar
  const [isOpen, setIsOpen] = useState(false); 
  const pathname = usePathname();

  // 2. Efecto para controlar la apertura según el tamaño de la pantalla
  useEffect(() => {
    // Si la pantalla es de tamaño Tablet o PC (>= 768px), abrimos el menú por defecto
    if (window.innerWidth >= 768) {
      setIsOpen(true);
    }
  }, []);

  // 3. Efecto para cerrar el menú en móviles al cambiar de página
  useEffect(() => {
    // Cada vez que 'pathname' cambia, verificamos si es móvil para ocultar el Sidebar
    if (window.innerWidth < 768) {
      setIsOpen(false);
    }
  }, [pathname]); // <-- Este array de dependencias hace la magia

  const isActive = (path: string) => pathname === path;

  return (
    <>
      {/* GATILLADOR: BOTÓN HAMBURGUESA FLOTANTE */}
      {!isOpen && (
        <button
          type="button"
      ...
      {/* CAPA DE FONDO OSCURO (SÓLO PARA MÓVILES) */}
      {isOpen && (
        <div 
      ...
      >
        ...
        {/* ENLACES DE NAVEGACIÓN (Limpiamos los onClick repetitivos) */}
        <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto custom-scrollbar whitespace-nowrap">
          ...
          <Link href="/" className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${isActive("/") ? "bg-brand-accent text-brand-navy shadow-md" : "text-gray-300 hover:bg-white/10 hover:text-white"}`}>
            <Megaphone size={18} className={isActive("/") ? "text-brand-navy" : "text-brand-accent"} />
            <span>Novedades y Anuncios</span>
          </Link>
          
          <Link href="/mis-pagos" className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${isActive("/mis-pagos") ? "bg-brand-accent text-brand-navy shadow-md" : "text-gray-300 hover:bg-white/10 hover:text-white"}`}>
          ...
          {userRole === "ADMIN" && (
            <>
              ...
              <Link href="/admin/configuracion" className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${isActive("/admin/configuracion") ? "bg-white text-brand-navy shadow-md" : "text-gray-300 hover:bg-white/10 hover:text-white"}`}>
                <Settings size={18} className={isActive("/admin/configuracion") ? "text-brand-navy" : "text-gray-400"} />
                <span>Configurar Año Escolar</span>
              </Link>
              
              <Link href="/admin/usuarios" className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${isActive("/admin/usuarios") ? "bg-white text-brand-navy shadow-md" : "text-gray-300 hover:bg-white/10 hover:text-white"}`}>
                <ShieldCheck size={18} className={isActive("/admin/usuarios") ? "text-brand-navy" : "text-gray-400"} />
                <span>Accesos y Permisos</span>
              </Link>
              
              <Link href="/admin/alumnos" className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${isActive("/admin/alumnos") ? "bg-white text-brand-navy shadow-md" : "text-gray-300 hover:bg-white/10 hover:text-white"}`}>
                <Users size={18} className={isActive("/admin/alumnos") ? "text-brand-navy" : "text-gray-400"} />
                <span>Directorio Alumnos</span>
              </Link>

              <Link href="/admin/ingresos" className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${isActive("/admin/ingresos") ? "bg-white text-brand-navy shadow-md" : "text-gray-300 hover:bg-white/10 hover:text-white"}`}>
                <Wallet size={18} className={isActive("/admin/ingresos") ? "text-brand-navy" : "text-gray-400"} />
                <span>Control de Ingresos</span>
              </Link>
              
              <Link href="/admin/egresos" className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${isActive("/admin/egresos") ? "bg-white text-brand-navy shadow-md" : "text-gray-300 hover:bg-white/10 hover:text-white"}`}>
                <Receipt size={18} className={isActive("/admin/egresos") ? "text-brand-navy" : "text-gray-400"} />
                <span>Registro de Egresos</span>
              </Link>

              <Link href="/admin/consultas" className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${isActive("/admin/consultas") ? "bg-white text-brand-navy shadow-md" : "text-gray-300 hover:bg-white/10 hover:text-white"}`}>
                <MessageSquare size={18} className={isActive("/admin/consultas") ? "text-brand-navy" : "text-gray-400"} />
                <span>Bandeja de Consultas</span>
              </Link>

              <Link href="/admin/encuestas" className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${isActive("/admin/encuestas") ? "bg-white text-brand-navy shadow-md" : "text-gray-300 hover:bg-white/10 hover:text-white"}`}>
